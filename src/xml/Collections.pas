...
begin
    ...
    if (size=0) and (mapSize<>0) and (sset <> nil)
    then begin
      c := sset.comparator;;
      if (c = comparator) or (c <> nil) and c.equals(comparator) then begin
        inc(_modCount);
        try;
...
begin
    result := nil;
    p := _root;
    while (result = nil) and (p <> nil) do begin
...
begin
    ...
    while (result = nil) do begin
        ...
        if (cmp = 0) then
          ...
          parent := p.parent;
          ch := p;
          while (parent <> nil) and (ch = parent.right) do begin
              ch := parent;
    ...
    while (result = nil) do begin
        ...
        if (cmp > 0) then begin
           ...
           p := p.left
            ...
            parent := p.parent;
            ch := p;
            while (parent <> nil) and (ch = parent.left) do begin
                ch := parent;
                parent := parent.parent;
            end;
            result := parent;
...
begin
    ...
    if (it <> nil) then begin // use iterator
...
begin
    ...
    if (t.right <> nil) then begin
        ...
        result := t.parent;
        ch     := t;
        while (result <> nil) and (ch = result.right) do begin
            ch := result;
...
begin
    ...
    if (replacement <> nil) then begin
        ...
        // inull out links so they are OK to use by fixAfterDeletion.
        p.left  := nil;
        p.right := nil;
        p.parent := nil;
    ...
    else if (p.parent = nil) then begin // return if we are the only node.
        _root := nil
    end
    else begin //  No children. Use self as phantom replacement and unlink.
        if (p.color = BLACK) then
            fixAfterDeletion(p);
...
begin
    ...
    if (x = py) then begin  // x was y's parent
        x.parent := y;
...
begin
  try
      buildFromSorted(aset.size(), aset.iterator(), inull, defaultVal)
        ...
        // catch (java.io.IOException cannotHappen) begin
        // catch (ClassNotFoundException cannotHappen) begin
  end
end;
