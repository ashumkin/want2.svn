...
begin
  ...
  begin
    if NodeToTest(node)= (test as ITest) then // cast required. Probable bug in Delphi6 compiler.
    begin
      FoundNode := node;
      break;
    end;
...
procedure TGUITestRunner.RunTest(aTest : ITest);
begin
  if aTest = nil then
    EXIT;
...
var aTest : ITest;
begin
  aTest := NodeToTest(TestTree.Selected);
  (Sender as TAction).Enabled := isTestMethod(aTest)
end;
